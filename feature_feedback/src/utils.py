...
def load_pokec(dataset,sens_attr,predict_attr, path="../dataset/pokec/", label_number=1000,sens_number=500,seed=19,test_idx=False):
    ...
    # build graph
    idx = np.array(idx_features_labels["user_id"], dtype='int64')
    idx_map = {j: i for i, j in enumerate(idx)}
    edges_unordered = np.genfromtxt(os.path.join(path,"{}_relationship.txt".format(dataset)), dtype='int64')

    edges = np.array(list(map(idx_map.get, edges_unordered.flatten())),
                     dtype='int64').reshape(edges_unordered.shape)
    ...
    idx_sens_train = torch.LongTensor(idx_sens_train[:(int(sens_number * len(idx_sens_train)))])
