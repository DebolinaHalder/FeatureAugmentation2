...
class Feature_generator(nn.Module):
    ...
    def __init__(self, nfeat,x,adj,rep, args):
        ...
        self.optimizer_A = torch.optim.Adam(self.adv.parameters(), lr = args.lr2)
    ...
    ##################################################
        ...
        ##################################################
        # ========== (2) INNER LOOP (GNN update) =========
        ##################################################

        # Freeze graph + feature generator
        for p in self.adj.parameters():
            p.requires_grad = False
        for p in self.adv.parameters():
            p.requires_grad = False

        k_inner = getattr(self.args, "inner_steps", 3)
