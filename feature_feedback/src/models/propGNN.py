...
class PropGNN(nn.Module):
    ...
    def optimize(self,g,x,labels,idx_train,sens,idx_sens_train,pred_sens):
        self.train()
        ...
        pred_sens = torch.reshape(pred_sens,(-1,1))
        y_score = torch.sigmoid(y)
        s_g_score = torch.sigmoid(s_g)
        #s_g_score = (s_g_score > 0.5).float()
        self.cov =  torch.abs(torch.mean((pred_sens - torch.mean(pred_sens)) * (y_score - torch.mean(y_score))))


        self.cls_loss = self.criterion(y[idx_train],labels[idx_train].unsqueeze(1).float())
        
        self.adv_loss = self.criterion(s_g_score,pred_sens.float())                
        ...
        s_g_score = torch.sigmoid(s_g)
        self.A_loss = self.criterion(s_g_score,pred_sens.float())
